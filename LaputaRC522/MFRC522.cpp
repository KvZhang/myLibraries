...
/*
unsigned char MFRC522::readOneByte(unsigned char blockAddr, unsigned char offsetAddr, char *returnAddr,MIFARE_Key *key){
	// Look for new cards
	...
	// Select one of the cards
		...
		status= this->PCD_Authenticate(MFRC522::PICC_CMD_MF_AUTH_KEY_A, blockAddr,
				key, &(this->uid));
		if (status != MFRC522::STATUS_OK)
			return 0;

		status = this->MIFARE_Read(blockAddr, buffer, &address);
		if (status != MFRC522::STATUS_OK)
			return 0;
		*returnAddr = buffer[offsetAddr];

		delay(10);
		this->PICC_HaltA(); // Halt PICC
		this->PCD_StopCrypto1();  // Stop encryption on PCD

		return 1;

}
unsigned char MFRC522::writeOneByte(unsigned char blockAddr, unsigned char offsetAddr, char data,MIFARE_Key *key){
	// Look for new cards
		if (!this->PICC_IsNewCardPresent())
			return 0;
	// Select one of the cards
		...
		status = this->MIFARE_Write(blockAddr, buffer, 16);
		if (status != MFRC522::STATUS_OK)
			return 0;
...
unsigned char MFRC522::readBlock(unsigned char blockAddr, char *returnAddr, unsigned char len,MIFARE_Key *key){
	// Look for new cards
		if (!this->PICC_IsNewCardPresent())
			return 0;
	// Select one of the cards
...
unsigned char MFRC522::writeBlock(unsigned char blockAddr, char *dataAddr, unsigned char len,MIFARE_Key *key){
	// Look for new cards
		if (!this->PICC_IsNewCardPresent())
			return 0;
	// Select one of the cards
