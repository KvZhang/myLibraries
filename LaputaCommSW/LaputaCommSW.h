...
{
	/*0x00*/
	SCAN_SLAVE = 0,	//广播寻找从机，仅主机可用，使用主机地址，从机ACK时需重复此命令，使用自己地址及回报类型
	/*0x01*/
	REPORTING_ERROR,	//读取从机的错误信息
	/*0x02*/
	READ_STATE,			//读取从机当前状态
	/*0x03*/
	STOP_REPORT,	//停止主动汇报型从机的汇报行为, if data!=0 then stop report.
	/*0x04*/
...
public:
	...
	bool cmdHandler(rxContent* rev);	//返回0时，说明是系统
